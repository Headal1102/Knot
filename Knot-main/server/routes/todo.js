...
router.post('/addSection', (req, res) => {
    ...
    const userId = '00'; // 예시 사용자 ID

    if (todosection && userId) {
        const query = 'INSERT INTO Todo (TodoCg, userId) VALUES (?, ?)';
        connection.execute(query, [todosection, userId], (error, results) => {
            if (error) {
...
router.post('/dropSection', (req, res) => {
    ...
    if (todosection && userId) {
        ...
        connection.execute(deleteQuery, [todosection, userId], (err, results) => {
            if (err) {
...
router.post('/changeSection', (req, res) => {
    ...
    const userId = '00'; // 예시 사용자 ID
    const todoDate = new Date().toISOString().split('T')[0]; // 현재 날짜를 기본값으로 설정
    ...
    connection.execute(updateQuery, [newSection, oldSection, userId], (err, results) => {
        ...
        // 수정된 섹션으로 변경된 후, 중복된 섹션이 있을 경우 삭제
        const deleteQuery = 'DELETE FROM Todo WHERE TodoCg = ? AND userId = ? AND todoDate = ?';
        connection.execute(deleteQuery, [oldSection, userId, todoDate], (deleteErr, deleteResults) => {
            if (deleteErr) {
...
router.post('/addTodo', (req, res) => {
    ...
    const userId = '00'; // 예시 사용자 ID
    const todoDate = new Date().toISOString().split('T')[0]; // 현재 날짜
    const todoCheck = 'n'; // 기본 체크 상태는 '미완료'
    ...
    connection.execute(query, [todoText, todoCheck, todoDate, userId, todoCg], (err, results) => {
        if (err) {
...
router.post('/editTodo', (req, res) => {
    ...
    connection.execute(query, [newTodoText, oldTodoText, userId, todoCg], (err, results) => {
        if (err) {
...
router.post('/deleteTodo', (req, res) => {
    ...
    connection.execute(query, [todoText, userId, todoCg], (err, results) => {
        if (err) {
...
router.post('/toggleTodoCheck', (req, res) => {
    ...
    connection.execute(currentCheckQuery, [todoText, userId, todoCg], (err, rows) => {
        ...
        if (rows.length > 0) {
            ...
            connection.execute(updateQuery, [newCheck, todoText, userId, todoCg], (err, results) => {
                if (err) {
